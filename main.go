...
var usrAgents = []string{
	"Mozilla/5.0 (X11; Linux i686; rv:104.0) Gecko/20100101 Firefox/104.0",
	"Mozilla/5.0 (X11; Linux x86_64; rv:104.0) Gecko/20100101 Firefox/104.0",
	"Mozilla/5.0 (X11; Ubuntu; Linux i686; rv:104.0) Gecko/20100101 Firefox/104.0",
...
func getUserAgents() string {
	...
	randNum := rand.Int() % len(usrAgents)
	fmt.Println("1. ", usrAgents[randNum])
	return usrAgents[randNum]
}
func buildGoogleURL(searchTerm, languageCode, countryCode string, pages, count int) ([]string, error) {
	toScrape := []string{}
	searchTerm = strings.Trim(searchTerm, " ")
	searchTerm = strings.Replace(searchTerm, " ", "+", -1)
	if gURL, found := googleDomains[countryCode]; found {
		for i := 0; i < pages; i++ {
			start := i * count
			scrapURL := fmt.Sprintf("%s%s&num=%d&hl=%s&start=%d&filter=0", gURL, searchTerm, count, languageCode, start)
			fmt.Println("2.", scrapURL)
			toScrape = append(toScrape, scrapURL)
		}
	} else {
		err := fmt.Errorf("(%s) is not supported", countryCode)
		return nil, err
	}
	fmt.Println("3.", toScrape)
	return toScrape, nil
}
func scrapeClientRequest(searchURL string, proxyString interface{}) (*http.Response, error) {
	baseClient := getScrapClient(proxyString)
	req, _ := http.NewRequest("GET", searchURL, nil)
	req.Header.Set("User-Agent", getUserAgents())
...
func main() {
	res, err := GoogleScrape("akhil sharma", "en", "com", nil, 1, 30, 10)
	fmt.Println(err)
	if err == nil {
